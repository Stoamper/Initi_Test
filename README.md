...
![badge](https://img.shields.io/badge/algorithm-description-blue)
...
•	Светофоры обрабатывают события параллельно, независимо друг от друга. При этом каждый светофор обрабатывает свои события последовательно, в том порядке, в каком они помещаются в очередь.
...
•	В исходной задаче указано, что автомобили при проезде перекрестка едут либо прямо, либо направо. Это значит, что можно синхронизировать работу автомобильных светофоров вдоль одной дороги (улицы). Если бы автомобилям надо было поворачивать налево, то пришлось бы закладывать время выдержки для пропуска противоположного потока.
...
Циклограммы - это диаграммы, показывающие время и последовательность выполнения операций в процессе. В данном репозитории приложены [циклограммы работы светофоров](https://github.com/Stoamper/Initi_Test/blob/main/Cyclograms.xlsx). Для их просмотра необходимо нажать "View raw".
...
Рассмотрим некоторые примеры, содержащие описание адаптивных алгоритмов работы светофоров. Для удобства каждому светофору присвоим уникальный id. Также объединим пары светофоров, которые относятся к одной дороге и одному пешеходному переходу.
